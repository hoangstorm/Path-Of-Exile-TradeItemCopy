// ==UserScript==
// @name         PathOfExile Trade Offical Item
// @namespace    http://tampermonkey.net/
// @description  PathOfExile Trade to Path of Building
// @source       https://github.com/fikal/PoeTradeItemCopy
// @author       HoangStorm
...
waitForKeyElements(".resultset", addCopyLinks);

function addCopyLinks() {
    var ids = [],
        i = 0;
    $(".resultset").bind("DOMNodeInserted DOMNodeRemoved", function () {
        var rows = $('.resultset .row');

        rows.each(function () {
            if ($(this).attr('data-id') && ids.indexOf($(this).attr('data-id')) < 0) {
                var that = $(this),
                    id = $(this).attr('data-id'),
                    link = $('<a>').attr('data-id', id).addClass('copy2POB btn btn-default').text('Copy');
                ...
                $(this).find(".btns .pull-left").append("&nbsp;");
                $(this).find(".btns .pull-left").append($('<span>').html(link));
            }
        });
    });
}
...
function PoeItem(div) {
    ...
    this.returnItemSockets = function () {
        var sockets = "";
        var find = this.htmlDiv.find(".sockets div");
        var tmpSocketLinks = [" ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "];
        var tmpSockets = [];
        for (var i = 0; i < find.length; i++) {
            ...
            if (classes.indexOf('socketLink') >= 0) {
                var socketLinkIndex = Number(classes[1].replace("socketLink", ""));
                tmpSocketLinks[socketLinkIndex * 2 + 1] = "-";
                sockets += "-";
            } else {
                if (this.regex.exec(sockets)) {
                    sockets += " ";
                }
                if (classes.indexOf('socketInt') >= 0) {
                    tmpSockets.push("B");
                    sockets += "B";
                } else if (classes.indexOf('socketDex') >= 0) {
                    tmpSockets.push("G");
                    sockets += "G";
                } else if (classes.indexOf('socketStr') >= 0) {
                    tmpSockets.push("R");
                    sockets += "R";
                } else if (classes.indexOf('socketGen') >= 0) {
                    tmpSockets.push("W");
                    sockets += "W";
                }
            }
        ...
        for (var j = 0; j < tmpSockets.length; j++) {
            tmpSocketLinks[j * 2] = tmpSockets[j];
        }

        return tmpSocketLinks.join("");
    };
    ...
    this.returnItemLevel = function () {
        return this.htmlDiv.find(".displayProperty span[data-field='ilvl'] .colourDefault").text().replace("ilvl:", "").trim();
    };

    this.returnRarity = function () {
        var rarityIndex = 0;
        var itemContainer = this.htmlDiv.find(".itemPopupContainer");
        if (itemContainer.hasClass("normalPopup")) {
            rarityIndex = 0;
        } else if (itemContainer.hasClass("magicPopup")) {
            rarityIndex = 1;
        } else if (itemContainer.hasClass("rarePopup")) {
            rarityIndex = 2;
        } else if (itemContainer.hasClass("uniquePopup")) {
            rarityIndex = 3;
        } else if (itemContainer.hasClass("gemPopup")) {
            rarityIndex = 4;
        } else if (itemContainer.hasClass("currencyPopup")) {
            rarityIndex = 5;
        } else if (itemContainer.hasClass("relicPopup")) {
            rarityIndex = 9;
        }
